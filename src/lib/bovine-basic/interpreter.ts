...
function evalNumericBinaryExpression(lhs: NumberVal, rhs: NumberVal, operator: string): NumberVal {
    let res = 0;
    if (operator == "+") {
...
function evalBinaryExpression(binOp: BinaryExpr, env: Environment): RuntimeVal {
    ...
    if (lhs.type == "number" && rhs.type == "number") {
        return evalNumericBinaryExpression(lhs as NumberVal, rhs as NumberVal, binOp.operator);
    }
