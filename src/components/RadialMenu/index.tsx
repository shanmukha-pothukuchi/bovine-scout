...
class RadialMenu extends Component<RadialMenuProps, RadialMenuState> {
    ...
    componentDidUpdate(previousProps: Readonly<RadialMenuProps>, previousState: Readonly<RadialMenuState>) {
    ...
    private handleCenterClick() {
        this.setState((state) => {
            ...
            const originalDepth = this.getMaxDepth(this.getVisibleMenu(this.props.menu, state.activePath));

            const newPath = state.activePath.slice(0, -1);
            while (newPath.length > 0) {
                const newDepth = this.getMaxDepth(this.getVisibleMenu(this.props.menu, newPath));
    ...
    private handleItemClick(id: MenuItem['id'], item: MenuItem | null) {
        ...
        this.setState((state) => {
            ...
            if (isLeaf &&
                this.getItemById(this.props.menu, state.activePath[state.activePath.length - 1])?.children
    ...
    private getMenuParams(menu: MenuItem[]): Map<MenuItem['id'], MenuParams> {
        ...
        const { baseRadius, radiusStep, centerRadius, maxLayerArcLength, maxItemArcLength } = this.props.style as MenuStyles;
        ...
        const stack: MenuNode[] = menu.map((item) => ({ item, parent: null, depth: 0 }));

        while (stack.length > 0) {
            const node = stack.pop()!;

            const outerRadius = node.depth === 0 ? baseRadius : baseRadius + node.depth * radiusStep;
            const innerRadius = node.depth === 0 ? centerRadius : outerRadius - radiusStep;

            const parentParams = node.parent ? menuItemArcParams.get(node.parent.id)! : null;
            ...
            if (node.depth === 0) {
                ...
                menuAngleSpan = Math.min(items.length * Math.min(360 / items.length, maxItemAngle), maxLayerAngle);
            }

            const parentCenterAngle = parentParams ? parentParams.arc.startAngle + parentParams.arc.sweepAngle / 2 : 0;
            const anglePerItem = menuAngleSpan / items.length;
            const angleStart = parentCenterAngle - (node.depth === 0 ? anglePerItem / 2 : menuAngleSpan / 2);
    ...
    private getArcPath({ centerX, centerY, innerRadius, outerRadius, startAngle, sweepAngle }: ArcParams): string {
